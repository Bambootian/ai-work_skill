...
2. **Update.** `/plugin update <name>` per plugin (or marketplace update); `npx @fission-ai/openspec@latest update` for OpenSpec projects. Record versions after.
...
- Fallback: a SessionStart hook that warns when `.claude/last-toolchain-refresh` is older than 30 days.
